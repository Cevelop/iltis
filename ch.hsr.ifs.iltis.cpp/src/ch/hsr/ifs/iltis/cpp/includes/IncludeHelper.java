...
public class IncludeHelper {
  ...
  public static TextFileChange createIncludeIfNotJetIncluded(final IASTTranslationUnit ast, final String includeName) {
    ...
    if (!isAlreadyIncluded(includeStatements, includeName)) { // $NON-NLS-1$
      final TextFileChange change = createTextFileChange(ast.getOriginatingTranslationUnit().getFile(),
          createInsertEdit(lastNode, includeName));
      return change;
    } else {
      return null;
    }
   ...
   *         already included.
   */
  private static boolean isAlreadyIncluded(final IASTPreprocessorIncludeStatement[] includeStatements,
      final String header) {
  ...
  private static int getInsertOffset(final IASTNode lastNode) {
    return lastNode == null ? 0
        : lastNode.getNodeLocations()[0].getNodeOffset() + lastNode.getNodeLocations()[0].getNodeLength();
  }
