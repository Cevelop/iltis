...
public class NodeWidget extends Composite {
   ...
   private void displayMethods(final TreeItem parent, final IASTNode node) {
      collectMethods(parent, node);
   }
   ...
   private void collectMethods(final TreeItem parentItem, final IASTNode node) {
      if (node == null || node.getClass() == null) return;
      ...
      for (final Method method : clazz.getMethods()) {
         if (Modifier.isPublic(method.getModifiers())) {
            if (method.getName().startsWith("get")) {
               try {
                  createTreeItem(parentItem, method.getName() + ";" + "[ " + method.getReturnType().getSimpleName() + " ] " + safeToString(method
                        .invoke(node)));
               } catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
      ...
      if (superclass != Object.class) {
         ...
         collectMethods(supermethods, parentNode);
         supermethods.setExpanded(true);
      }
   }

   private void collectMethods(final TreeItem parentItem, final Class<?> binding) {
      if (binding == null || binding.getClass() == null) return;
      Class<? extends Object> clazz = binding.getClass();
   ...
   private void displayBindings(final TreeItem parent, final IASTNode node, final IBinding binding) throws CoreException {
      ...
      collectMethods(methods, binding.getClass());
