...
import ch.hsr.ifs.iltis.testing.highlevel.testingplugin.cdttest.base.CDTTestingTest;
import ch.hsr.ifs.iltis.testing.highlevel.testingplugin.cdttest.base.projectholder.FakeProjectHolder;
import ch.hsr.ifs.iltis.testing.highlevel.testingplugin.cdttest.base.projectholder.ITestProjectHolder;
import ch.hsr.ifs.iltis.testing.highlevel.testingplugin.cdttest.base.projectholder.TestProjectHolder;
...
public abstract class CDTTestingCheckerTest extends CDTTestingTest {
    ...
    private void extractMarkerLines(final Properties properties) {
        ...
        if (markerLines != null && !markerLines.isEmpty()) {
            expectedMarkerLinesFromProperties = Stream.of(markerLines.split(",")).map(String::trim).map(Integer::valueOf).collect(Collectors
                    .toList());
        }
    }
    ...
    private void assertMarkerAttributes(final String attributeName, String failMessage, IMarker[] markers, List<String> expectedValues)
        ...
        assertTrue("Not all expected values for attribute " + attributeName + " found. Remaining: " + StringUtil.toString(expecetedValuesMutable),
                expecetedValuesMutable.isEmpty());
    }
