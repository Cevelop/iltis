...
public final class CompositeMarkerInfo extends MarkerInfo<CompositeMarkerInfo> {
    ...
    public Map<String, String> toMap() {
        final Map<String, String> markerInfoData = super.toMap();
        markerInfoData.putAll(InfoConverter.convert(infos));
        return markerInfoData;
    }

    void loadInfos(final MutableList<MarkerInfo<?>> infos) {
        this.infos.addAll(infos.collect(this::copyArguments));
    }

    private MarkerInfo<?> copyArguments(final MarkerInfo<?> info) {
        return info.also(c -> {
            c.fileName = fileName;
            c.severity = severity;
            c.message = message;
...
class CompositeMarkerInfoConverter extends InfoConverter {
    ...
    protected static <R extends IInfo<R>, T> MutableList<R> createInfos(final Map<String, T> data) {
        final MutableMap<String, String> infosData = Maps.adapt(data).keyValuesView().select(p -> p.getOne().startsWith(INFO_DATA_PREFIX)).toMap(
                p -> p.getOne().substring(INFO_DATA_PREFIX.length()), p -> (String) p.getTwo());

        return infosData.keyValuesView().collect(p -> MarkerInfo.fromCodanProblemArgsArray(ILTISException.sterilize(() -> {
            final String[] infoElement = p.getOne().split(INFO_TYPE_SEPARATOR);
